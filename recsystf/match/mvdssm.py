...
class MVDSSMEstimator(tf.estimator.Estimator):
    def __init__(self,
        ...
        def custom_model_fn(features, labels, mode, params=None, config=None):
            ...
            tf.logging.info(
                "%s MVDSSMEstimator custom_model_fn, user_item_sim.shape:%s" %
                (
            ...
            with tf.name_scope(item_type):
                ...
                tf.logging.info(
                    "%s MVDSSMEstimator custom_model_fn, logits.shape:%s" %
                    (
                        time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                        logits.shape
                    )
                ...
                eval_metric_ops[item_type + "_auc"] = tf.metrics.auc(labels, predictions)
        ...
        tf.logging.info(
            "[%s] MVDSSMEstimator init" % time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        )
